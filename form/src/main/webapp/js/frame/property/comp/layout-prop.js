...
LayoutRatioProp.prototype = {
     ...
     */
    getHtml: function() {
        var placeholder = "分隔比例为1-12之间的数字，逗号分隔和为12";
        ...
        return "<input id=\"" + this.id + "\" type=\"text\" value=\"" + this.getProperty() + "\" placeholder=\"" + placeholder + "\" " + disabled + " class=\"form-textbox form-textbox-text col-md-12\"/>";
    },
    ...
    setProperty: function(event, currentPropertyID) {
        if (event.type == "focusout") {
            ...
            if (oldColumnSize > newColumnSize && this.isColumnsHasChildItem(oldColumns, newColumnSize)) {
 ...
 * 布局器样式属性
 * */
var LayoutStyleProp = function(componentObject) {
    this.componentObject = componentObject;
    this.registerEvent();
};
 ...
 * 设置默认样式
 */
LayoutStyleProp.setDefaultStyle = function() {
    var $selectedComp = $(".form-component_active"),
        $lyrow = $selectedComp,
        $layout,
        defaultStyle = "layout-white";

    if($selectedComp.parent().parent().hasClass('demo')){
        $selectedComp.children(':first')
            .addClass('layout layout-white')
            .attr("layout-body-style", "layout-white");
        $("#layout_body_style").val("layout-white");    
    }   

    if ($selectedComp.parents(".lyrow").length <= 2) {
        return;
    }

    var level = 0;
    $.each($selectedComp.parents(".lyrow"), function(index, item) {
        level++;
        if ($(item).find("[type=bfd_panel]:first").length > 0) {
            if (level <= 2) {
                defaultStyle = "layout-default";
            }

            return false;
        }

        if ($(item).find("[type=tab]:first").length > 0) {
            if (level <= 3) {
                defaultStyle = "layout-default";
            }
    ...
    if (!$lyrow.hasClass('lyrow')) {
        $lyrow = $selectedComp.parents(".lyrow:first");
    }

    $layout = $lyrow.find("[type=layout]:first,[type=bfd_panel]:first,[type=tab]:first");
    if($layout.attr("type") === "bfd_panel"){
        return;
    }

    if($layout.attr("type") === "tab"){
        defaultStyle = "layout-white";
    }

    $.each(LayoutStyleProp.Style, function(key, value) {
        $layout.removeClass(key);
    });

    $layout.addClass(defaultStyle);
    $layout.attr("layout-body-style", defaultStyle);
    $("#layout_body_style").val(defaultStyle);
};
 ...
 * 布局器样式配置
 * */
LayoutStyleProp.prototype = {

    /**
     * 获取组件html
     * */
    getHtml: function() {
        var html = ['<select id="layout_body_style" class="form-textbox form-textbox-text col-md-12">'],
            selectedStyle = this._getProperty();

        $.each(LayoutStyleProp.Style, function(index, item) {
            var selected = "";
    ...
    registerEvent: function() {
        var $that = this;
        $('.properties .form-panel-body').off("change", "#layout_body_style")
            .on('change', '#layout_body_style', function(e) {
                e.stopPropagation();
                e.preventDefault();

                $.each(LayoutStyleProp.Style, function(key, value) {
                    $that.componentObject.removeClass(key);
                });

                $that.componentObject.addClass($(this).val());
                $that.componentObject.attr("layout-body-style", $(this).val());
            });
    },
    ...
    _getProperty: function() {
        var style = this.componentObject.attr("layout-body-style");
        if (!style) {
            style = "layout-default";
            this.componentObject.attr("layout-body-style", style);
        }
...
LayoutDataSourceProp.prototype = {
    ...
    getDataSourceHtml: function(selectedProperty, disabled) {
        ...
        $.each(dataSources, function(index, item) {
            ...
            html.push("<input dstype=\"" + item.dsType + "\" name=\"" + that.dataSourceId + "\" type=\"radio\" value=\"" + item.id + "\" " + checked + " " + disabled + "/>" + item.name);
        })
    ...
    getDataSetHtml: function(selectedProperty, parentLayoutProperty) {
        ...
        if (parentLayoutProperty && parentLayoutProperty.uri) {
            ...
            if (bfd_set_ref) {
                options.push("<option bfd_set_type=\"" + bfd_set_type + "\" bfd_set_ref=\"" + bfd_set_ref + "\" value=\"" + parentLayoutProperty.uri + "\" " + selected + " >" + parentLayoutProperty.uri + "(" + bfd_set_ref + ")</option>");
            } else {
                options.push("<option bfd_set_type=\"" + bfd_set_type + "\" value=\"" + parentLayoutProperty.uri + "\" " + selected + " >" + parentLayoutProperty.uri + "</option>");
            }
        }
        ...
        if (dataSets && dataSets.length > 0) {
            $.each(dataSets, function(index, item) {
                ...
                if (bfd_set_ref) {
                    options.push("<option bfd_set_type=\"" + bfd_set_type + "\" bfd_set_ref=\"" + bfd_set_ref + "\" value=\"" + item.name + "\" " + selected + " >" + item.name + "(" + bfd_set_ref + ")</option>");
                } else {
                    options.push("<option bfd_set_type=\"" + bfd_set_type + "\" value=\"" + item.name + "\" " + selected + " >" + item.name + "</option>");
                }
            })
        }
