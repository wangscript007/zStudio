	...
	FrameGuide.prototype = {
		...
		getGuideParam: function() {
			...
			steps.push({
				...
				tipWidth: '240px',
				text: '这里是<span class="guide-highlight">隐藏组件</span>，此类组件设计阶段不会被执行，在运行期才会被调用执行',
				beforeShow: function() {
					$("#layoutCompPanel").prev(".body").hide();
					$("#layoutCompPanel .body").hide();
					$("#baseCompPanel .body").hide();
	...
	FormLayout.prototype = {
		...
		getGuideParam: function() {
			...
			steps.push({
				...
				to: "tr:contains(布局比例)",
				tipWidth: '300px',
				text: '设置<span class="guide-highlight">布局比例</span>，布局比例为1-12之间的数值，比例之和大于0小于12，比例值之间以逗号分隔；如：4,4,4'
			});
			...
			steps.push({
				...
				to: "tr:contains(数据源)",
				tipWidth: '300px',
				text: '绑定<span class="guide-highlight">数据源</span>，布局组件可多层嵌套，如果在其中一层绑定了数据源，则其子组件将会继承父组件数据源'
			});
			...
			steps.push({
				...
				tipWidth: '300px',
				text: '添加<span class="guide-highlight">组件</span>，拖曳基础组件到布局组件中'
			});
