 ...
 * 通过脚本对象，加载列表数据
 */
function initVerticalListObject()
{
	var attrbutes=attributeAPI.getAttributesByTypeAndAttrName("type","vertical_list","datasetting");
	if(attrbutes.length>0){
		for(var i=0;i<attrbutes.length;i++){
			var item = attrbutes[i];
			var dataScriptObject = ($.parseJSON(escape2Html(decodeURIComponent(item.value)))).script;
			...
			if(data == undefined || data.length == 0){
				if(typeof(loadCustomDataScriptObject) != "undefined") {
					...
					console.error("initVerticalListObject fail. dataScriptObject convert to data not success. loadCustomDataScriptObject function is undefined.")
					return;
				}
			}

			if(data == undefined) {
				console.error("initVerticalListObject fail. data is undefined.");
				return;
			}

			var verticalListObject=$("div[compid="+item.compid+"]").find("ul:first");
			if(data.length > 0){
				verticalListObject.empty();
				for(var j=0;j<data.length;j++){
					for(var obj in data[j])	{ 
						var iconHtml = " ";
						if(obj.split('--').length>0){
							debugger;
							iconHtml = '<span class="' +data[j][obj].split("--")[1]+ '" style="margin-right:3px;top:2px"></span>';
							verticalListObject.append('<li><a href="javascript:;" data-value=' + data[j][obj].split("--")[0] + '>'+  iconHtml +obj.split("--")[0]+' </a></li>');
						}else{
							verticalListObject.append('<li><a href="javascript:;" data-value=' + data[j][obj] + '>'+  iconHtml +obj+' </a></li>');
						}
				...
				$.each(result,function(key,value){
					var index =0;
					var verticalListName = "";
					var verticalListValue = "";
					for(var item in value){
						if(index == 0){
							verticalListName = value[item];
						}else if(index == 1){
							verticalListValue = value[item];
						}
						index++;
					}
					verticalListObject.append('<li><a href="javascript:;" data-value=' + verticalListValue + '>'+verticalListName+' </a></li>');
					//selectObject.append("<option value=\""+optionValue+"\">"+optionName+"</>");
				});
	...
	if(jQuery(".jquery-accordion-menu").jqueryAccordionMenu) {
...
function initTimeComponent() {
    ...
    var operator = getUrlParam('operator', window.location.search);
	if(operator && operator === 'add') {
		$.each($('.form_time'), function (index, item) {
			var time = getCurrentTime();
			var $item = $(item),
				defaultvalue = $item.parent().data('defaultvalue'),
				vtype = $item.parent().attr('vtype');
			...
			if (_dTools && _dTools.formatTime) {
				var temp = _dTools.formatTime(time);
				if (vtype === 'date') {
					$item.val(temp.substring(0, 10));
				}
				else {
					$item.val(temp);
				}
				$item.datetimepicker('update');
			}
		})
	}
}
		 ...
		 $(this).fileinput({
			...
			ajaxSettings:{
				success:function(data){
					 ...
					 * 文件上传成功后回调
					 * */
					if (typeof(eval("window." + id + "SuccessCallBack")) !== "undefined") {
						applyFunc(id + "SuccessCallBack", [data]);
					}
				}
			}
		});
...
/**
	初始化页面中未加载的树
*/
function initZtree(dsname,vmObject){
	$("#"+vmObject.$id).find(".ztree").each(function(){
...
function initButtonOperationEvent() {
	...
	$.each($("button[bfd-button-query]"), function (index, item) {
		...
		$(item).off('click').on('click', function () {
			var dsType = $("[avalonctrl=" + property.modelId + "]").attr("dstype") || "orm";
			if (dsType === "orm") {
				queryRemoteTable(property.tableId,
					$.bfd.pageCache.queryCondition.getOrmQueryConditions(eval(property.modelId)));
			} else {
				var url = getTableOptions(property.tableId, "url");
				if (url && url.indexOf("?") > -1) {
					url = url.substring(0, url.indexOf("?") + 1);
				}

				var conditions = $.bfd.pageCache.queryCondition.getJSONTypeQueryConditions(eval(property.modelId));
				if (conditions) {
					if (url && url.indexOf("?") == -1) {
						url += "?";
					}
				...
				refreshTable(property.tableId, {url: url});
			}
		})
 ...
 * 选择组件初始化
 */
function initAdvancedSelect(vmObject) {
	var advancedSelect = $("#"+vmObject.$id).find("[type=advanced_select]");
	if (advancedSelect.length > 0) {
		$.each(advancedSelect, function (index, item) {
			var field = $(item).attr("field");
			$(item).bfdAdvancedSelect("setNames", vmObject[field]);
		})
	}
}
...
function initComponent(dsname,vmObject) {
	...
	initAdvancedSelect(vmObject);
	initCheckBox(vmObject);
	initMultipleSelect(vmObject);
	initImage(vmObject);
	initVerticalListObject();
}
...
;(function ($, win) {
	...
	//表单页面状态控制
	var formComponentStatus = function () {
		if (!win.viewoperator && !pageParams) {
			return;
		}
		
		var operator,container="body";
		if(pageParams){
			operator = getUrlParam("operator",pageParams);
			operator = operator === 'edit'?'modify':operator;
			container=".modal-dialog";
		}else{
			operator = win.viewoperator === 'edit' ? 'modify' : win.viewoperator;
		}
		
		$.each($(container).find('div[editable^="%7B%22"]'), function (index, item) {			
			var $item = $(item),
				editAttr = JSON.parse(decodeURIComponent($item.attr('editable'))),
				status = editAttr[operator] === 'checked' ? true : false;
			$item.children().prop('disabled', !status);
		});
	}


	function pageCssControl() {
		var modelParents = $('.container-top').parents(".modal-body");
		if(modelParents[0]) {
			return;
		}
