;(function($,win) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
                if (!$.bfd.pageCache.execOperationSuccessFuc(viewModel, tableName, "GET", dsname)) {
                    ...
                    that.analyzeData(viewModel.$model, retData[0]);
                }
            },
     ...
     * @returns {Array}
     */
    CustomJsonPageModel.prototype.getData = function (dsName, tableName, columns, conditions, viewModel) {
        var dataRows = [],
            url = getOperatorURL(dsname, tableName);
    ...
    CustomJsonPageModel.prototype._saveVMData = function (viewModel, uri, method, dsname) {
        ...
        //11.20 临时修改
        var newData = applyFunc(viewModel.$id + "ParameterFilterCallBack", [data]);
        ...
        $.ajax({
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                if (!$.bfd.pageCache.execOperationErrorFuc(viewModel, uri, method, dsname)) {
                    if (typeof(eval("window." + viewModel.$id + "ErrorCallBack")) == "undefined" &&
                        typeof(eval("window.vmErrorCallBack")) == "undefined") {
                        errorCallback(textStatus, errorThrown, url);
                    } else {
                        applyFunc(viewModel.$id + "ErrorCallBack", [textStatus]);
                        //1120临时修改
