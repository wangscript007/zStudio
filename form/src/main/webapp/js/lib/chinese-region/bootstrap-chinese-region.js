...
(function(root,factory){
	...
	var ChineseRegion = function($el){
		...
		this._defaultId = "";
		this.$el = $el;
		this.$input = this.$el.find('input[type=text]:first');
		this.$inputName = this.$input.next();
		this.$inputText = this.$inputName.next();
	...
	ChineseRegion.prototype.onAreasChanged = function(e) {
		...
		this.$input.val(this.areas[this.areas.length-1].id);
		this.$inputName.val(this.$inputText.val());
		this.$el.trigger('changed.bs.chinese-region',[this.areas]);
	};

	ChineseRegion.prototype.formatArea = function(areaName){
		if(!areaName){
			return "";
		}

		return $.trim(areaName).replace("省", "")
			.replace("回族自治区", "")
			.replace("维吾尔自治区", "")
			.replace("壮族自治区", "")
			.replace("自治区", "")
	...
	ChineseRegion.prototype.renderAreasPanel = function(data,first) {
		...
		for (var i = 0; i < this.areas.length; i++) {
			this.$inputText.val(this.$inputText.val()+this.areas[i].name+' ');
			this.$tabContent.find('a[data-id='+this.areas[i].id+']').addClass('current');
		}

		this.$inputText.val(this.formatArea(this.$inputText.val()));
	};
	/**
	 * 重绘
	 */
