...
/**
 * Extends java.util.BigInteger to provide zeroization mechanism. 
 */
public class SecureBigInteger extends Number implements Comparable<SecureBigInteger> {
	
	/// --- added by hu.jianghui

	/**
	 * Determine if current instance has been zeroized.
	 */
	public boolean isZeroized(){
		if (signum != 0)
			return false;
		for (int i=0; mag != null && i<mag.length; i++){
			if (mag[i] != 0){
				return false;
			}
		}
	...
	/**
	 * Miller-Rabin Probabilistic Primality Test FIPS 186-3
	 */
	public boolean millerRabinTest186_3(int iterations, Random rand){
		// Find a and m such that m is odd and this == 1 + 2**a * m
        SecureBigInteger thisMinusOne = this.subtract(ONE);
		SecureBigInteger m = thisMinusOne;
		int a = m.getLowestSetBit();
		m = m.shiftRight(a);

		// Do the tests
		for (int i=0; i<iterations; i++){
			// Generate a uniform random on (1, this-1)
			SecureBigInteger b;
			do {
				b = new SecureBigInteger(this.bitLength(), rand);
			} while (b.compareTo(ONE) <= 0 || b.compareTo(this.subtract(ONE)) >= 0);
			SecureBigInteger z = b.modPow(m, this);
			if (z.equals(ONE) || (z.equals(this.subtract(ONE)))){
				continue;
			}
			boolean flag = false;
			for (int j=0; j<a-1; j++){
				...
				if (z.equals(ONE)){
					return false;
				}else if (z.equals(this.subtract(ONE))){
					flag = true;
					break;
				}
			}
     ...
     */
    public SecureBigInteger(byte[] val) {
    	this(1,val);
    }
     ...
     */
    public static SecureBigInteger createRandomInRange(
    	SecureBigInteger      min,
    	SecureBigInteger      max,
        Random    random)
    {
        ...
        {
            SecureBigInteger x = new SecureBigInteger(max.bitLength(), random);
        ...
        return new SecureBigInteger(max.subtract(min).bitLength() - 1, random).add(min);
    }
     ...
     */
    private SecureBigInteger(int[] val) {
	if (val.length == 0)
	    throw new NumberFormatException("Zero length BigInteger");
     ...
     */
    public SecureBigInteger(int signum, byte[] magnitude) {
     ...
     */
    private SecureBigInteger(int signum, int[] magnitude) {
     ...
     * @see    Character#digit
     */
    public SecureBigInteger(String val, int radix) {
     ...
     * @see    Character#digit
     */
    public SecureBigInteger(String val) {
	this(val, 10);
    }
     ...
     * @see #bitLength()
     */
    public SecureBigInteger(int numBits, Random rnd) {
     ...
     * @see    #bitLength()
     */
    public SecureBigInteger(int bitLength, int certainty, Random rnd) {
        SecureBigInteger prime;
     ...
     * @since 1.4
     */
    public static SecureBigInteger probablePrime(int bitLength, Random rnd) {
	if (bitLength < 2)
	    throw new ArithmeticException("bitLength < 2");
     ...
     */
    private static SecureBigInteger smallPrime(int bitLength, int certainty, Random rnd) {
        ...
        while(true) {
            ...
            SecureBigInteger p = new SecureBigInteger(temp, 1);
    ...
    private static final SecureBigInteger SMALL_PRIME_PRODUCT
     ...
     * test.
     */
    private static SecureBigInteger largePrime(int bitLength, int certainty, Random rnd) {
        SecureBigInteger p;
        p = new SecureBigInteger(bitLength, rnd).setBit(bitLength-1);
        p.mag[p.mag.length-1] &= 0xfffffffe;

        // Use a sieve length likely to contain the next prime number
        int searchLen = (bitLength / 20) * 64;
        BitSieve searchSieve = new BitSieve(p, searchLen);
        SecureBigInteger candidate = searchSieve.retrieve(p, certainty, rnd);

        while ((candidate == null) || (candidate.bitLength() != bitLength)) {
            p = p.add(SecureBigInteger.valueOf(2*searchLen));
    ...
    * @since 1.5
    */
    public SecureBigInteger nextProbablePrime() {
        if (this.signum < 0)
        ...
        SecureBigInteger result = this.add(ONE);
        ...
        while(true) {
           ...
           SecureBigInteger candidate = searchSieve.retrieve(result,
						 DEFAULT_PRIME_CERTAINTY, null);
           if (candidate != null)
               return candidate;
           result = result.add(SecureBigInteger.valueOf(2 * searchLen));
        }
    }
     ...
     */
    public boolean passesLucasLehmer() {
        SecureBigInteger thisPlusOne = this.add(ONE);
        ...
        // Step 2
        SecureBigInteger u = lucasLehmerSequence(d, thisPlusOne, this);
     ...
     */
    private static int jacobiSymbol(int p, SecureBigInteger n) {
        if (p == 0)
            return 0;
	...
	if ((p & u & 2) != 0)	// p = u = 3 (mod 4)?
	    j = -j;
	// And reduce u mod p
	u = n.mod(SecureBigInteger.valueOf(p)).intValue();
    ...
    private static SecureBigInteger lucasLehmerSequence(int z, SecureBigInteger k, SecureBigInteger n) {
        SecureBigInteger d = SecureBigInteger.valueOf(z);
        SecureBigInteger u = ONE; SecureBigInteger u2;
        SecureBigInteger v = ONE; SecureBigInteger v2;
	...
	for (int i=0; i<iterations; i++) {
	    ...
	    SecureBigInteger b;
	    do {
		b = new SecureBigInteger(this.bitLength(), rnd);
     ...
     */
    private SecureBigInteger(int[] magnitude, int signum) {
     ...
     */
    private SecureBigInteger(byte[] magnitude, int signum) {
	this.signum = (magnitude.length==0 ? 0 : signum);
        this.mag = stripLeadingZeroBytes(magnitude);
    }
     ...
     */
    SecureBigInteger(MutableBigInteger val, int sign) {
     ...
     */
    public static SecureBigInteger valueOf(long val) {
	// If -MAX_CONSTANT < val < MAX_CONSTANT, return stashed constant
	if (val == 0)
	    return ZERO;
	// commented by hujianghui, valueOf() can return a const value which can be zeroized.
//	if (val > 0 && val <= MAX_CONSTANT)
//	    return posConst[(int) val];
//	else if (val < 0 && val >= -MAX_CONSTANT)
//	    return negConst[(int) -val];

	return new SecureBigInteger(val);
    }

    /**
     * Constructs a BigInteger with the specified value, which may not be zero.
     */
    private SecureBigInteger(long val) {
        if (val < 0) {
            signum = -1;
            val = -val;
        } else {
     ...
     */
    private static SecureBigInteger valueOf(int val[]) {
        return (val[0]>0 ? new SecureBigInteger(val, 1) : new SecureBigInteger(val));
    }
    ...
    private static SecureBigInteger posConst[] = new SecureBigInteger[MAX_CONSTANT+1];
    private static SecureBigInteger negConst[] = new SecureBigInteger[MAX_CONSTANT+1];
    static {
	for (int i = 1; i <= MAX_CONSTANT; i++) {
	    int[] magnitude = new int[1];
	    magnitude[0] = (int) i;
	    posConst[i] = new SecureBigInteger(magnitude,  1);
	    negConst[i] = new SecureBigInteger(magnitude, -1);
	}
    }
     ...
     * @since   1.2
     */
    public static final SecureBigInteger ZERO = new SecureBigInteger(new int[0], 0);
     ...
     * @since   1.2
     */
    public static final SecureBigInteger ONE = valueOf(1);

    /**
     * The BigInteger constant two.  (Not exported.)
     */
    private static final SecureBigInteger TWO = valueOf(2);
     ...
     * @since   1.5
     */
    public static final SecureBigInteger TEN = valueOf(10);
     ...
     */
    public SecureBigInteger add(SecureBigInteger val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
	...
	if (val.signum == signum)
            return new SecureBigInteger(add(mag, val.mag), signum);
        ...
        return new SecureBigInteger(resultMag, cmp*signum);
    }
     ...
     * @return <tt>this - val</tt>
     */
    public SecureBigInteger subtract(SecureBigInteger val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
     ...
     * @return <tt>this * val</tt>
     */
    public SecureBigInteger multiply(SecureBigInteger val) {
        ...
        return new SecureBigInteger(result, signum*val.signum);
    }
     ...
     */
    private SecureBigInteger square() {
        if (signum == 0)
	    return ZERO;
        int[] z = squareToLen(mag, mag.length, null);
        return new SecureBigInteger(trustedStripLeadingZeroInts(z), 1);
    }
     ...
     */
    public SecureBigInteger divide(SecureBigInteger val) {
        ...
        a.divide(b, q, r);
        return new SecureBigInteger(q, this.signum * val.signum);
    }
     ...
     * @throws ArithmeticException <tt>val==0</tt>
     */
    public SecureBigInteger[] divideAndRemainder(SecureBigInteger val) {
        SecureBigInteger[] result = new SecureBigInteger[2];
        MutableBigInteger q = new MutableBigInteger(),
                          r = new MutableBigInteger(),
                          a = new MutableBigInteger(this.mag),
                          b = new MutableBigInteger(val.mag);
        a.divide(b, q, r);
        result[0] = new SecureBigInteger(q, this.signum * val.signum);
        result[1] = new SecureBigInteger(r, this.signum);
        return result;
    }
     ...
     * @throws ArithmeticException <tt>val==0</tt>
     */
    public SecureBigInteger remainder(SecureBigInteger val) {
        MutableBigInteger q = new MutableBigInteger(),
                          r = new MutableBigInteger(),
                          a = new MutableBigInteger(this.mag),
                          b = new MutableBigInteger(val.mag);
        ...
        a.divide(b, q, r);
        return new SecureBigInteger(r, this.signum);
    }
	...
	return new SecureBigInteger(result, newSign);
    }
     ...
     */
    public SecureBigInteger gcd(SecureBigInteger val) {
        if (val.signum == 0)
	    return this.abs();
        ...
        return new SecureBigInteger(result, 1);
    }
	...
	return new SecureBigInteger(this.mag, -this.signum);
    }
     ...
     * @see    #remainder
     */
    public SecureBigInteger mod(SecureBigInteger m) {
	if (m.signum <= 0)
	    throw new ArithmeticException("BigInteger: modulus not positive");

	SecureBigInteger result = this.remainder(m);
     ...
     * @see    #modInverse
     */
    public SecureBigInteger modPow(SecureBigInteger exponent, SecureBigInteger m) {
	if (m.signum <= 0)
	    throw new ArithmeticException("BigInteger: modulus not positive");
	...
	SecureBigInteger base = (this.signum < 0 || this.compareTo(m) >= 0
			   ? this.mod(m) : this);
	...
	if (m.testBit(0)) { // odd modulus
	    ...
	    SecureBigInteger m1 = m.shiftRight(p);  // m/2**p
	    SecureBigInteger m2 = ONE.shiftLeft(p); // 2**p

            // Calculate new base from m1
            SecureBigInteger base2 = (this.signum < 0 || this.compareTo(m1) >= 0
                                ? this.mod(m1) : this);

            // Caculate (base ** exponent) mod m1.
            SecureBigInteger a1 = (m1.equals(ONE) ? ZERO :
                             base2.oddModPow(exponent, m1));

	    // Calculate (this ** exponent) mod m2
	    SecureBigInteger a2 = base.modPow2(exponent, p);

	    // Combine results using Chinese Remainder Theorem
	    SecureBigInteger y1 = m2.modInverse(m1);
	    SecureBigInteger y2 = m1.modInverse(m2);
     ...
     */
    private SecureBigInteger oddModPow(SecureBigInteger y, SecureBigInteger z) {
    /*
	...
	buf = 0;
        ...
        return new SecureBigInteger(1, t2);
    }
     ...
     */
    private SecureBigInteger modPow2(SecureBigInteger exponent, int p) {
	/*
	 * Perform exponentiation using repeated squaring trick, chopping off
	 * high order bits as indicated by modulus.
	 */
	SecureBigInteger result = valueOf(1);
	SecureBigInteger baseToPow2 = this.mod2(p);
        int expOffset = 0;
     ...
     */
    private SecureBigInteger mod2(int p) {
	if (bitLength() <= p)
	    return this;
	...
	return (mag[0]==0 ? new SecureBigInteger(1, mag) : new SecureBigInteger(mag, 1));
    }
     ...
     */
    public SecureBigInteger modInverse(SecureBigInteger m) {
	if (m.signum != 1)
	    throw new ArithmeticException("BigInteger: modulus not positive");
     ...
     * @see #shiftRight
     */
    public SecureBigInteger shiftLeft(int n) {
        if (signum == 0)
            return ZERO;
        if (n==0)
            return this;
        ...
        return new SecureBigInteger(newMag, signum);
    }
     ...
     * @see #shiftLeft
     */
    public SecureBigInteger shiftRight(int n) {
        if (n==0)
            return this;
        if (n<0)
            return shiftLeft(-n);
     ...
     * @return <tt>this &amp; val</tt>
     */
    public SecureBigInteger and(SecureBigInteger val) {
     ...
     * @return <tt>this | val</tt>
     */
    public SecureBigInteger or(SecureBigInteger val) {
	int[] result = new int[Math.max(intLength(), val.intLength())];
	for (int i=0; i<result.length; i++)
	    result[i] = (int) (getInt(result.length-i-1)
				| val.getInt(result.length-i-1));
     ...
     * @return <tt>this ^ val</tt>
     */
    public SecureBigInteger xor(SecureBigInteger val) {
	int[] result = new int[Math.max(intLength(), val.intLength())];
	for (int i=0; i<result.length; i++)
	    result[i] = (int) (getInt(result.length-i-1)
				^ val.getInt(result.length-i-1));
     ...
     * @return <tt>this &amp; ~val</tt>
     */
    public SecureBigInteger andNot(SecureBigInteger val) {
	int[] result = new int[Math.max(intLength(), val.intLength())];
	for (int i=0; i<result.length; i++)
	    result[i] = (int) (getInt(result.length-i-1)
				& ~val.getInt(result.length-i-1));
     ...
     * @throws ArithmeticException <tt>n</tt> is negative.
     */
    public SecureBigInteger setBit(int n) {
	if (n<0)
	    throw new ArithmeticException("Negative bit address");
     ...
     * @throws ArithmeticException <tt>n</tt> is negative.
     */
    public SecureBigInteger clearBit(int n) {
	if (n<0)
	    throw new ArithmeticException("Negative bit address");
     ...
     * @throws ArithmeticException <tt>n</tt> is negative.
     */
    public SecureBigInteger flipBit(int n) {
	if (n<0)
	    throw new ArithmeticException("Negative bit address");
     ...
     */
    public int compareTo(SecureBigInteger val) {
	...
	if (!(x instanceof SecureBigInteger))
	    return false;
	SecureBigInteger xInt = (SecureBigInteger) x;
     ...
     */
    public SecureBigInteger min(SecureBigInteger val) {
     ...
     *         <tt>val</tt>.  If they are equal, either may be returned.
     */
    public SecureBigInteger max(SecureBigInteger val) {
	return (compareTo(val)>0 ? this : val);
    }
	...
	SecureBigInteger tmp = this.abs();
	int numGroups = 0;
	while (tmp.signum != 0) {
            SecureBigInteger d = longRadix[radix];
            ...
            a.divide(b, q, r);
            SecureBigInteger q2 = new SecureBigInteger(q, tmp.signum * d.signum);
            SecureBigInteger r2 = new SecureBigInteger(r, tmp.signum * d.signum);
    ...
    private static SecureBigInteger longRadix[] = {null, null,
...
class BitSieve {
     ...
     * be even.
     */
    BitSieve(SecureBigInteger base, int searchLen) {
        /*
     ...
     */
    SecureBigInteger retrieve(SecureBigInteger initValue, int certainty, java.util.Random random) {
        ...
        for (int i=0; i<bits.length; i++) {
            ...
            for (int j=0; j<64; j++) {
                if ((nextLong & 1) == 1) {
                    SecureBigInteger candidate = initValue.add(
                                           SecureBigInteger.valueOf(offset));
...
class MutableBigInteger {
     ...
     */
    MutableBigInteger(SecureBigInteger b) {
    ...
    private final int getLowestSetBit() {
        ...
        if (b==0) 
            return -1;
        return ((intLen-1-j)<<5) + SecureBigInteger.trailingZeroCnt(b);
    }
     ...
     */
    public String toString() {
        SecureBigInteger b = new SecureBigInteger(this, 1);
        return b.toString();
    }
    ...
    void rightShift(int n) {
        ...
        if (nBits == 0)
            return;
        int bitsInHighWord = SecureBigInteger.bitLen(value[offset]);
    ...
    void divideOneWord(int divisor, MutableBigInteger quotient) {
        ...
        int shift = 32 - SecureBigInteger.bitLen(divisor);
    ...
    void divide(MutableBigInteger b,
        ...
        int shift = 32 - SecureBigInteger.bitLen(d[0]);
        if (shift > 0) {
            // First shift will not grow array
            SecureBigInteger.primitiveLeftShift(d, dlen, shift);
            // But this one might
            rem.leftShift(shift);
        }
    ...
    static int binaryGcd(int a, int b) {
        ...
        int y = SecureBigInteger.trailingZeroTable[x];
        aZeros += y;
        a >>>= y;
        ...
        y = SecureBigInteger.trailingZeroTable[x];
        bZeros += y;
        b >>>= y;
        ...
        while (a != b) {
            if ((a+0x80000000) > (b+0x80000000)) {  // a > b as unsigned
                ...
                while ((x = (int)a & 0xff) == 0)
                    a >>>= 8;
                a >>>= SecureBigInteger.trailingZeroTable[x];
            } else {
                b -= a;

                while ((x = (int)b & 0xff) == 0)
                    b >>>= 8;
                b >>>= SecureBigInteger.trailingZeroTable[x];
            }
        }
        return a<<t;
    }
...
class SignedMutableBigInteger extends MutableBigInteger {
   ...
   /**
     ...
     */
    public String toString() {
        SecureBigInteger b = new SecureBigInteger(this, sign);
        return
            b.toString();
    }
