...
ProjectDataModel.prototype = {
	...
	getProjectInfo : function(projectName) {
		var result;
	...
	getDataSourceList : function(projectName) {
		var result;
		if (ProjectDataModel.projectInfo) {
			$.each(ProjectDataModel.projectInfo, function(index, item) {
				if (item.name == projectName) {
					result = ProjectDataModel.formatDataSourceData(item.sourceItem, projectName);
					return false;
				}
			});
		}
	...
	getDataSourceInfo : function(projectName, datasourceId) {
		var result;
		var dataSources = this.getDataSourceList(projectName);
		if (dataSources) {
	...
	getProjectDesignFileList : function(projectName) {
		var result = [];
		if (ProjectDataModel.designFiles) {
			var project = this.getProjectInfo(projectName);
	...
	getDesignFileInfo : function(projectName, fileFullInfo) {
		var result;
		var files = this.getProjectDesignFileList(projectName);
		if (files.length > 0) {
...
function ProjectConfigViewManager() {
	...
	this.operator = ProjectManagerUI.OPERATOR_VIEW;
...
ProjectConfigViewManager.prototype.init = function() {
	...
	this.moduleType = getUrlParam("moduleType", href) || this.moduleType;
	this.operator = getUrlParam("operator", href) || this.operator;
	this.projectName = getUrlParam("projectName",href) || this.projectName;
...
ProjectConfigViewManager.prototype.setCurrentView = function(moduleType, operator, projectName, nodeId) {
	...
	switch (this.moduleType){
		case ProjectManagerUI.MODULE_TYPE_PROJECT:
			this.currentView = new ProjectView(this.operator);
			this.currentViewData = projectInfo;
			break;
		case ProjectManagerUI.MODULE_TYPE_DATASOURCELIST:
			...
			break;
		case ProjectManagerUI.MODULE_TYPE_DATASOURCE:
			...
			break;
		case ProjectManagerUI.MODULE_TYPE_DESIGNFILELIST:
			...
			break;
		case ProjectManagerUI.MODULE_TYPE_DESIGNFILE:
...
ProjectConfigViewManager.prototype.refreshView = function(){
	...
	this.setCurrentView (
		...
		ProjectManagerUI.OPERATOR_VIEW,
...
ProjectConfigViewManager.prototype.reloadParentProjectParam = function(parentPageProjectParam){
	...
	if(window.parent.workbench.projectManager.getCurrentProject() != parentPageProjectParam.currentProject){
		return;
	}
	/*重新加载父页面参数*/
	window.parent.workbench.projectManager.loadProjectFile(
		parentPageProjectParam.currentProject,
...
ProjectView.prototype = {
	...
	setVisibility : function() {
		...
		$("#btnProjectAdd").show();
		$("#btnSave").show();
		if (this.operator == ProjectManagerUI.OPERATOR_VIEW) {
			$("#btnSave").hide();
		}
		...
		if (!parent.window.isFrameProjectUpdateEnable) {
	...
	setEditableStatus : function() {
		var disabled = this.operator == ProjectManagerUI.OPERATOR_VIEW;
		vm1438050062200.input_text_project_local_path_form_disabled = disabled;
		vm1438050062200.input_text_project_publish_path_form_disabled = disabled;
		vm1438050062200.input_text_project_common_js_path_form_disabled = disabled;
		vm1438050062200.input_text_project_common_css_path_form_disabled = disabled;
		vm1438050062200.input_text_preview_port_form_disabled = disabled;
		vm1438050062200.input_checkbox_is_i18n_disabled = disabled;

		if (this.operator == ProjectManagerUI.OPERATOR_ADD) {
	...
	setData : function(data) {
		if (data && this.operator != ProjectManagerUI.OPERATOR_ADD) {
	...
	save : function(callback) {
		...
		if (webAppIndex > -1 ) {
			...
			//project.previewPrefix = "designer/layoutit";	//xh 20161108:此次是针对BCP这种基于ICT的版本的处理，因为ict中没有webapps目录, 数据可视化展示不考虑ict
		}
	...
	getParentPageProjectParam:function(){
		...
		parentPageProjectParam.currentFile = window.parent.workbench.projectManager.getCurrentFile();
		parentPageProjectParam.currentFileName = window.parent.workbench.projectManager.getCurrentFileName();
	...
	getChangedProjectDesignFileName:function(){
		var fileFullName = window.parent.workbench.projectManager.getCurrentFile();
		if(fileFullName == ""){
			return "";
		}
		fileFullName = fileFullName;
		var newLocalPath = getFormatedFilePath(vm1438050062200.input_text_project_local_path);
		var oldLocalPath = getFormatedFilePath(window.parent.workbench.projectManager.getCurrentProjectLocalPath());
...
projectDesignFileView.prototype = {
	...
	isProjectDesignFileExists:function(){
		return window.workbench.REST_API.isProjectDesignFileExists(this.getProjectDesignFileName());
	},
	save:function(callback){
		if (!this.validate()) {
			return ;
		...
		if (this.operator == ProjectManagerUI.OPERATOR_ADD) {
			window.workbench.REST_API.createFile(this.getProjectDesignFileName(), callbackFun);
		} else if (this.operator == ProjectManagerUI.OPERATOR_UPDATE) {
			window.workbench.REST_API.renameFile(vm1438050062200.fileOldFullName, this.getProjectDesignFileName(), callbackFun);
		}
	},
	saveAndOpen:function(callback) {
		...
		var eventhandler = function () {
			...
			window.parent.workbench.projectManager.loadProjectFile(pname, fileFullPath);
...
$(document).ready(function(){
	...
	bootstrapTable('table_base_local_design_file', adjustTableParam('','undefined', 'undefined', {"striped":true,"pagination":true,"pageSize":"50","pageList":[50,100,200],"height":450,"search":true,"showColumns":false,"showRefresh":false,"sidePagination":"client","sortable":true,"idField":"$id","columns":[{"field":"$id","visible":false,"title":"ID","class":"td-word-wrap"},{"field":"fileName","title":"文件名","formatter":"","editable":false,"visible":true,"validate":"","class":"td-word-wrap"},{"field":"fileRelativePath","title":"相对路径","formatter":"","editable":false,"visible":true,"validate":"","class":"td-word-wrap"},{"field":"modifyTime","title":"修改时间","formatter":"ProjectDesignFileListView.designFileDateTimeRowFormat","editable":false,"visible":true,"validate":"","class":"td-word-wrap","width":200},{"field":"","title":"操作","formatter":"ProjectDesignFileListView.designFileDeleteRowFormat","editable":false,"visible":true,"validate":"","class":"td-word-wrap"}]}));
})