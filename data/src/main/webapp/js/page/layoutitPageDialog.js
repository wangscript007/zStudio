...
function showModalDialog(params){
    ...
    if(!$dialog.data("ly-params")){
        ...
        $dialog.children('.modal-content').children('.modal-body').css("height","600px").css("overflow","auto");
    }else{
        var params = $dialog.data("ly-params");
        $dialog.css("width",params.compwidth);
        $dialog.children('.modal-content').children('.modal-body').css("height",params.compheight).css("overflow","auto");
        /* if(typeof params.btnevent !== "undefined"){
         $dialog.find(".modal-footer>button:first").click(function(){
         evalFunc(params.btnevent)
         });
         } */
