...
(function($, win) {
     ...
     * api
     */
    var Bar = function(el, schema) {
		this.el = el;
        this.schema = schema;
    };
	
	Bar.prototype.getHtmlElement = function() {
		return this.el[0];
	};
     ...
     * api
     */
    Bar.prototype.getDataSourceInfo = function() {
     ...
     * api
     */
	Bar.prototype.getQueryCondArr = function() {
     ...
     */
    Bar.prototype.getOption = function(rows) {
        var transformRule = {};
        transformRule.xAxis = {
    ...
    Bar.prototype._mixinRenderOption = function(dataOption) {
	...
	Bar.prototype._drawChart = function (option) {
        var chart = echarts.init(this.getHtmlElement());
        chart.setOption(option);
    };

    Bar.prototype._drawError = function (errorMessages) {
        var errorHtml = [];
        errorHtml.push('<div class="ksy-chart-err-wrap">');
        errorHtml.push('<div class="ksy-chart-err">');
        errorHtml.push('<div class="ksy-chart-err-title">该【柱状图】存在以下问题</div>');
	...
	$.fn.zdata_bar = function(options, param) {
		this.each(function () {
            var el = $(this);
			
			var plugin = new Bar(el, options);
