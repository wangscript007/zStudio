...
function getFormatCode() {
    ...
    $(downloadlayout).children().html($(frameHtmlObj).html());
...
function _adjustOrder(selectElement) {
    ...
    $.each(boxarr,
        function () {
            ...
            if ($(that).find('div[type="tab"]').length > 0) {
                ...
                $.each(lyrow,
                    function () {
                        var compid = $(this).find('.view>div[type="layout"]').attr('compid');
    ...
    $.each(hasExecuteLyrow,
        function () {
            lyrow.splice(jQuery.inArray(this, lyrow), 1);
        });
...
(function ($, win) {
    ...
    var _nodesHtml = function (nodes, dataMap) {
        ...
        html.push('<div type="layout" ratio="12" compid="' + vmId + '" compname="' + vmId + '" layoutstyle="" ms-controller="' + vmId + '" id="' + vmId + '" ' + bfdOperationParams + ' ' + attrStr + '>');
        ...
        html.push('<div class="preview">');
        html.push('<div class="img-left"><img src="img/frame/icon/ly-12.png" class="mCS_img_loaded"></div>12/自定义</div>');
        html.push('<div class="view draggableHandle">');
        html.push('<div type="layout" ratio="12" compid="' + layoutId + '" compname="' + layoutId + '" layoutstyle="" id="' + layoutId + '">');
        html.push('<div class="row clearfix">');
        html.push('<div class="col-md-12 col-xs-12 col-sm-12 col-lg-12 column ui-sortable" style="padding-bottom: 0px;"><div class="box box-element ui-draggable" style="display: block;">');
        ...
        html.push('<div class="view form-component_active draggableHandle">');
        html.push('<div type="button" data-color="btn-primary" data-size="btn-sm" compid="' + buttonId + '" compname="' + buttonId + '" class="layout-align-right" button_type_select="submit" buttontype="submit" defaultvalue="%E7%A1%AE%E5%AE%9A">');
        html.push('<button type="button" class="btn btn-primary btn-sm" id="' + buttonId + '" name="' + buttonId + '" ms-click="submit" i18nkey="确定">确定</button>');
        html.push('</div>');
        html.push('</div>');
        html.push('</div></div>');
        html.push('</div>');
    ...
    var _createDataHtml = function (nodeArry, dataMap) {
        ...
        html.push('<div type="layout" ratio="12" compid="' + vmodelId + '" compname="' + vmodelId + '" layoutstyle="" id="' + vmodelId + '" >');
        html.push('<div class="row clearfix">');
    ...
    var splitLayoutStyle = function (tArray, visibleFlag) {
        ...
        if (visibleFlag) {
            html.push('<div class="lyrow ui-draggable" style="display: block;">');
            html.push('<a href="#close" class="remove label label-danger showRemove">');
            html.push('<i class="glyphicon-remove glyphicon">');
            html.push('</i>');
            html.push('</a>');
            html.push('<div class="preview">');
            html.push('<div class="img-left">');
            html.push('<img src="img/frame/icon/ly-12.png" class="mCS_img_loaded">');
            html.push('</div>');
            html.push('12/自定义');
            html.push('</div>');
            html.push('<div class="view form-component_active draggableHandle">');
            html.push('<div type="layout" ratio="12" compid="layout' + copmid + '" compname="layout' + copmid + '"');
            html.push('layoutstyle="" id="layout' + copmid + '" componentvisibility="true">');
            html.push('<div class="row clearfix">');
            html.push('<div class="col-md-12 col-xs-12 col-sm-12 col-lg-12 column ui-sortable">');
        }
        for (var i = 0; i < tArray.length; i++) {
            if (tArray[i].length == 1) {
                ...
                if (tArray[i][0].layout != 1) {
                    html.push(_segmentationRow(tArray[i][0], undefined));
                } else {
                    html.push(_entireRow(tArray[i][0]));
                }
            } else {
                if ((JSON.stringify(tArray[i][1]) == "{}")) {
                    html.push(_segmentationRow(tArray[i][0], undefined));
                } else {
                    html.push(_segmentationRow(tArray[i][0], tArray[i][1]));
                }
    ...
    /*布局器2,4,2,4布局的页面代码拼接*/
    var _segmentationRow = function (node, nodeNext) {
        var html = [],
            layoutId = "layout" + new Date().getTime() + parseInt(1000 * Math.random());
        html.push('<div class="lyrow ui-draggable" style="display: block;">');
        html.push('<a href="#close" class="remove label label-danger">');
