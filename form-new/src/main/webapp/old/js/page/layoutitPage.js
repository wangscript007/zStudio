 ...
 */
function getVerticalList(dsname, table, name, value, options, condition, isDistinct,customVal) {
        ...
        success: function (data, status) {
            ...
            if(options != []){
                ...
                for(var i = 0; i<options.length;i++){
                    if(options[i].icon){
                        array[options[i].name+"--icon"] = options[i].value + "--" + options[i].icon;
                    }else{
                        array[options[i].name] = options[i].value;
                    }
                }
