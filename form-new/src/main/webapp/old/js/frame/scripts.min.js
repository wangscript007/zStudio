...
function componentStop(item) {
	...
	if ($(item).parent().hasClass("column")) {
		$(item).children("[href=\"#close\"]").addClass("showRemove");
		$(item).children("[href=\"#add\"]").addClass("showRemove");
	}
	...
	$(".demo,.demo .column").sortable({
		...
		stop: function (event, ui) {
			...
			if (currComponent.parent().hasClass('demo')) {
				...
				$demo.mCustomScrollbar({ theme: window.designFormGrolbalObject.demoMCustomScrollbarTheme });
				if (isScrollToBottom) {
 ...
 * @param selectColumn
 * @param selectComponentHeight
 */
function layoutResize(selectColumn, selectComponentHeight) {
	if (selectColumn != undefined && !selectColumn.hasClass("demo") &&
		...
		$(selectColumn).parent().children().each(function () {
			...
			if ($(selectColumn).html() == $(this).html()
				&& selectComponentHeight != undefined) {
				columnHeight = columnHeight - selectComponentHeight;
			}
		...
		$(selectColumn).parent().children().each(function () {
			...
			if (childSize == 0) {
				columnHeight = 0;
			} else if (childSize > 1 && columnHeight == 0) {
				//当删除按钮与其他组件在同一排时，不能减去组件高度。
				columnHeight = $(this).height();
			}

			var paddingBottom = maxHeight - columnHeight;
			$(this).css("padding-bottom", paddingBottom);
		});
		//移动端高度特殊处理
		//		$(".demo_parent").attr('style', 'height:' + ($(".demo").outerHeight(true) + 20) + 'px');
		//重新计算父容器列高
...
function initComponents() {
	...
	$('body.edit .demo').unbind("click").on("click", ".lyrow .view,.box .view", function (e) {
		...
		bootbox.confirm("确认要删除当前组件吗？", function (result) {
			if (result) {
				//组件删除后重新计算column高度
				layoutResize(currentObject.parent().parent(), currentObject.parent().height());
	...
	$(document).on("keydown", function (e) {
		...
		if(e.keyCode === 46 && $.inArray(e.target.nodeName,ignoredArray) === -1){
			var $removeObj = $(".demo .form-component_active,.footer-bg .form-component_active").parent().find(".remove:first");
			if($removeObj.length > 0){
				$removeObj.trigger('click');
			}			
		}		
	});
