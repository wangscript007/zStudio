...
function createTabContent(contentId,editorId,active){
    ...
    return [
        '<div role="tabpanel" class="tab-pane',active?' active':' ','" id="',contentId,'" >',
            '<div id="',editorId,'" class="edit"></div>',
        '</div>'
    ].join("");
...
function registerEvents(){
    $("#loadAceEditor").click(function(e){
        //一次选择一条记录
        var row = $("#filesTable").bootstrapTable("getSelections")[0];
        ...
        $.ajax({
            type: "get",
            url: getContextPath() + "/jersey-services/layoutit/file/get?filePath=" + path + "&fileName="+row["fileName"],
            dataType:"json",
            success: function (data, textStatus) {
