...
;(function($,win) {
	...
	DataSourceAPI.prototype = {
		 ...
		 * @returns {string}
		 */
		getDataSourceType: function (dataSourceId) {
			var dsType = "orm";
			var dataSource = this.getDataSourceById(dataSourceId);
			if(dataSource && !$.isEmptyObject(dataSource)){
				dsType = dataSource[DataSourceAPI.DATA_SOURCE_COLUMN.DSTYPE];
			}
		 ...
		 * @param dataFields
		 */
		updateDataSetFields:function(dataSourceId, dataServiceID, dataSetID,dataFields) {
			 ...
			 * @type {*|Array}
			 */
			var inDataFields = this.getDataSetFields(dataSourceId,
				dataServiceID, dataSetID, DataSourceAPI.DATA_SETS_PARAM_TYPE.IN);
			this._addDataSetFieldsViewName(inDataFields, dataFields);

			/**
			 * 更新输出字段
			 * @type {*|Array}
			 */
			var outDataFields = this.getDataSetFields(dataSourceId,
				dataServiceID, dataSetID, DataSourceAPI.DATA_SETS_PARAM_TYPE.IN);
			this._addDataSetFieldsViewName(outDataFields, dataFields);
		},
		 ...
		 * @param extendDataFields
		 */
		_addDataSetFieldsViewName:function(metaDataFields,extendDataFields) {
			if (!metaDataFields || metaDataFields.length < 0) {
				return;
			}

			if (!extendDataFields || extendDataFields.length < 0) {
				return;
			}

			$.each(metaDataFields, function (index, item) {
				for (var i = 0; i < extendDataFields.length; i++) {
					if (extendDataFields[i]["item_id"] === item[DataSourceAPI.DATA_FIELDS_COLUMN.NAME]) {
						item["displayName"] = extendDataFields[i]["item_name"];
						break;
					}
				}
			})
		 ...
		 * @returns {{}}
		 */
		getBFDOperationParam:function(dataSourceId, dataServiceID, dataSetID,msController) {
			var operations = this.getDataOperations(dataSourceId, dataServiceID, dataSetID),
				post_in_fields = this.getPostInFields(dataSourceId, dataServiceID, dataSetID),
				put_in_fields = this.getPutInFields(dataSourceId, dataServiceID, dataSetID),
				get_out_fields = this.getGetOutFields(dataSourceId, dataServiceID, dataSetID),
				model = msController,
				parameters = {},
				dsType = this.getDataSourceType(dataSourceId);

			if (operations.length > 0 || post_in_fields.length > 0 ||
				put_in_fields.length > 0 || get_out_fields.length > 0) {
				parameters = {
					"dsType": dsType,
					"model": model,
					"source": dataSourceId,
					"service": dataServiceID,
					"set": dataSetID,
					"operations": operations,
					"post_in": post_in_fields,
					"put_in": put_in_fields,
					"get_out": get_out_fields
				};
			}
